...
exports.up = function (knex, Promise) {
    ...
    return knex.schema.createTable('users', (table) => {
        ...
        table.integer('statistics_id').unsigned().references('user_statistics.id');
        ...
        table.increments('id').primary();
        table.integer('user_id').unsigned().notNullable().references('users.id');
        table.integer('book_id').unsigned().notNullable().references('books.id');
        ...
        table.increments('id').primary();
        table.integer('user_id').unsigned().notNullable().references('users.id');
        table.integer('challenge_id').unsigned().notNullable().references('challenges.id');
        table.string('state').notNullable();
        table.datetime('created_at');
    }).table('challenges', (table) => {

        table.integer('current_state_id').unsigned().notNullable().references('challenge_states.id');
        ...
        table.increments('id').primary();
        table.integer('user_id').unsigned().notNullable().references('users.id');
        table.integer('intention_id').unsigned().notNullable().references('intentions.id');
        table.string('state').notNullable();
        table.datetime('created_at');
    }).table('intentions', (table) => {

        table.integer('current_state_id').unsigned().notNullable().references('intention_states.id');
    });
};
