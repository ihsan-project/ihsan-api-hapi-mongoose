...
module.exports = class UserService extends Schmervice.Service {
    ...
    async createSession(payload, transaction) {
        ...
        let session = await UserSession.query(transaction).first().where({ authId: authentication.id });
        ...
        if (!session) {
            ...
            session = await UserSession.query(transaction).insert({
                ...
                token: this.createToken(authentication.userId),
                createdAt: new Date(),
    ...
    async createToken(id) {
        ...
        return await JWT.sign({ id }, this.options.jwt_secret_key, {
            ...
            expiresIn: '1m' // TODO: Need to corrolate this with `expiresAt`
        });
    }
};
